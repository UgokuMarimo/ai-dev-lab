  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white/90 backdrop-blur-md shadow-xs">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2.5 font-bold text-lg text-slate-900 hover:text-[#1b4332] transition-colors">
          <div className="relative h-9 w-9 overflow-hidden rounded-lg border border-[#2d6a4f]/30 shadow-xs shrink-0">
            <Image
        ...
        <nav className="hidden md:flex items-center gap-1">
          {NAV_ITEMS.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="px-3.5 py-2 text-sm font-semibold text-slate-700 hover:text-[#1b4332] hover:bg-[#e8f5e9] rounded-md transition-all"
            >
              {item.label}
            </Link>
          ))}
